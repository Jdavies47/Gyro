...
void loop() {
  ...
  float servoRollReactionAngV = servoRollAngVel(wheelRPM, abs(gx), angle); //This is the reaction torque from the bike roll only
  float rollPWMTimeDifference = pwmPeriodDiff(servoRollReactionAngV);
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////


  /////////////////////////////// This section is angle correction roll //////////////////////////////////////////
  float servoAngleReactionAngV = servoAngleAngV(abs(roll), wheelRPM, angle);
...
void sevroMove (int pulseSize){
  // Add code to limit servo movements, this should smooth movement as well as redxuce acceleration 
  int currentPulse = Servo1.read();
  if(pulsechangeisbigenough){
    if (DifferenceLimitnotexceed){
    //continue
    // Need to add some buffer code to start the smoothing process, and see if thi 
    }
    else
    {
     //reduce to max limit
